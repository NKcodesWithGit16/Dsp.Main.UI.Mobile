...
// react-native-maps excluded in Expo Go — enabled in production native builds
const MapView = null;
const Marker = null;
...
function normalizeDriver(d) {
  ...
  const st = typeof d.status === 'number' ? (statusMap[d.status] || 'offline') : (d.status || 'offline').toLowerCase();
  const name = d.name || (d.firstName || d.lastName ? `${d.firstName || ''} ${d.lastName || ''}`.trim() : 'Driver');
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.surface2 }}>
      ...
      <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, opacity: 0.15 }}>
        {[...Array(8)].map((_, i) => (
          <View key={`h${i}`} style={{ position: 'absolute', top: `${i * 14}%`, left: 0, right: 0, height: 1, backgroundColor: colors.border }} />
        ))}
        {[...Array(6)].map((_, i) => (
          <View key={`v${i}`} style={{ position: 'absolute', left: `${i * 20}%`, top: 0, bottom: 0, width: 1, backgroundColor: colors.border }} />
        ))}
      </View>
          ...
          >
            {isSelected && (
              <View style={{
                ...
                backgroundColor: colors.accent + '22', top: -7,
              }} />
            )}
            <View style={{
              width: isSelected ? 44 : 36, height: isSelected ? 44 : 36,
              borderRadius: 99, backgroundColor: colors.elevated,
              borderWidth: isSelected ? 2.5 : 1.5,
              borderColor: isSelected ? colors.accent : colors.border,
              justifyContent: 'center', alignItems: 'center',
            }}>
              <Text style={{ fontSize: 10, fontWeight: '700', color: colors.textPrimary }}>{d.initials}</Text>
              <View style={{ position: 'absolute', top: 2, right: 2, width: 8, height: 8, borderRadius: 99, backgroundColor: stColor, borderWidth: 1.5, borderColor: colors.elevated }} />
            </View>
            <Text style={{ color: colors.textMuted, fontSize: 9, marginTop: 2, fontWeight: '600' }}>{d.name.split(' ')[0]}</Text>
          </TouchableOpacity>
        );
      })}

      <View style={{ position: 'absolute', bottom: 14, right: 14, backgroundColor: colors.surface1 + 'ee', padding: 8, borderRadius: 10, borderWidth: 1, borderColor: colors.border }}>
        <Text style={{ color: colors.textMuted, fontSize: 10, textAlign: 'center' }}>🗺️ Live map in production build</Text>
      </View>
    </View>
  );
}
...
export default function DriversScreen() {
  ...
  // Simulate movement on mock data
  useEffect(() => {
  ...
  const sendMessage = async () => {
    ...
    setChatInput('');
    const newMsg = { id: Date.now().toString(), sender: 'dispatcher', text, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) };
      ...
      <View style={s.mapWrap}>
        {MapView ? (
          <MapView ref={mapRef} style={StyleSheet.absoluteFill} provider={PROVIDER_GOOGLE}
            ...
            {drivers.map(driver => (
              <Marker key={driver.id} coordinate={{ latitude: driver.lat, longitude: driver.lng }} onPress={() => selectDriver(driver)}>
          ...
          <MapPlaceholder drivers={filteredDrivers.slice(0, 8)} selectedDriver={selectedDriver} onSelect={selectDriver} colors={colors} />
        )}
          ...
          <View style={s.floatingCard}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={s.floatingStats}>
                {selectedDriver.eta !== '—' && <Text style={[s.floatingStat, { color: colors.textSecondary }]}>🕐 {selectedDriver.eta}</Text>}
                {selectedDriver.dist !== '—' && <Text style={[s.floatingStat, { color: colors.textSecondary }]}>📍 {selectedDriver.dist}</Text>}
              </View>
            </View>
            ...
            <TouchableOpacity onPress={() => setSelectedDriver(null)} style={s.floatingClose} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
      ...
      <View style={[s.sheet, sheetExpanded && s.sheetExpanded, {
        ...
        <View style={s.sheetHeader}>
          <Text style={[s.sheetTitle, { color: colors.textPrimary }]}>
            Drivers <Text style={{ color: colors.accent }}>({drivers.length})</Text>
          </Text>
          <View style={s.sheetActions}>
            <TouchableOpacity style={[s.iconBtn, { backgroundColor: colors.surface2 }]} onPress={() => setShowSearch(v => !v)}>
              <Text>🔍</Text>
            </TouchableOpacity>
        ...
        {apiLoading ? (
          ...
          <FlatList
            ...
            renderItem={({ item: d }) => (
              <TouchableOpacity
                style={[s.driverRow, selectedDriver?.id === d.id && { backgroundColor: colors.accentMuted }, { borderBottomColor: colors.borderSubtle }]}
                onPress={() => selectDriver(d)}
                activeOpacity={0.8}
              >
                <View style={[s.avatar, { backgroundColor: colors.surface2 }]}>
                  <Text style={[s.avatarText, { color: colors.textSecondary }]}>{d.initials}</Text>
                  <View style={[s.statusDot, { backgroundColor: STATUS_COLORS[d.status] || '#6b7280', borderColor: colors.surface1 }]} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={[s.driverName, { color: colors.textPrimary }]}>{d.name}</Text>
                  <Text style={[s.driverTruck, { color: colors.textMuted }]}>{d.truck}</Text>
                  {d.status !== 'offline' && (
                    <Text style={[s.driverRoute, { color: colors.textDisabled }]} numberOfLines={1}>{d.origin} → {d.dest}</Text>
                  )}
                </View>
                <View style={{ alignItems: 'flex-end', gap: 4 }}>
                  <View style={[s.statusBadge, { backgroundColor: (STATUS_COLORS[d.status] || '#6b7280') + '22' }]}>
                    <Text style={[s.statusBadgeText, { color: STATUS_COLORS[d.status] || '#6b7280' }]}>{d.status}</Text>
                  </View>
        ...
        <View style={{ flex: 1, backgroundColor: colors.pageBg }}>
          <LinearGradient colors={['#4f46e5', '#6366f1']} style={s.chatHeader}>
            <TouchableOpacity onPress={() => setChatVisible(false)}>
              <Text style={{ color: 'rgba(255,255,255,0.8)', fontSize: typography.base }}>← Back</Text>
            </TouchableOpacity>
          ...
          <FlatList
            ...
            renderItem={({ item: m }) => (
              <View style={[s.msgWrap, m.sender === 'dispatcher' ? s.msgRight : s.msgLeft]}>
                <View style={[s.msgBubble, m.sender === 'dispatcher'
                  ? { backgroundColor: colors.accentDark }
                  : { backgroundColor: colors.surface2, borderWidth: 1, borderColor: colors.border }
                ]}>
                  <Text style={[s.msgText, { color: m.sender === 'dispatcher' ? '#fff' : colors.textPrimary }]}>{m.text}</Text>
                  <Text style={[s.msgTime, { color: m.sender === 'dispatcher' ? 'rgba(255,255,255,0.55)' : colors.textDisabled }]}>{m.time}</Text>
                </View>
              </View>
            )}
          />
...
function AddDriverModal({ visible, onClose, colors, dispatcherId }) {
  ...
  const handleSend = async () => {
    ...
    setLoading(true);
    try {
      await inviteDriver(name, contact, dispatcherId);
    } catch {}
    setLoading(false);
    setSent(true);
  };
  ...
  const handleClose = () => {
    setSent(false); setName(''); setContact('');
    onClose();
  };
        ...
        <View style={[addS.modal, { backgroundColor: colors.elevated, borderColor: colors.border }]}>
          {sent ? (
            <View style={{ alignItems: 'center', padding: spacing[4] }}>
              <Text style={{ fontSize: 44, marginBottom: spacing[3] }}>✅</Text>
            ...
            <>
              ...
              <View style={{ marginTop: spacing[4], gap: spacing[3] }}>
                <TextInput style={[addS.input, { backgroundColor: colors.surface2, borderColor: colors.border, color: colors.textPrimary }]}
                  placeholder="Driver Name" placeholderTextColor={colors.textDisabled} value={name} onChangeText={setName} />
                <TextInput style={[addS.input, { backgroundColor: colors.surface2, borderColor: colors.border, color: colors.textPrimary }]}
                  placeholder="Email or Phone" placeholderTextColor={colors.textDisabled} value={contact} onChangeText={setContact}
              ...
              <View style={{ flexDirection: 'row', gap: spacing[2], marginTop: spacing[5] }}>
                ...
                <TouchableOpacity style={{ flex: 1 }} onPress={handleSend} disabled={loading}>
                  <LinearGradient colors={['#6366f1', '#4f46e5']} style={[addS.btn, { opacity: loading ? 0.6 : 1 }]}>
                    {loading ? <ActivityIndicator color="#fff" size="small" /> : <Text style={addS.btnText}>Send Invite</Text>}
                  </LinearGradient>
                </TouchableOpacity>
              </View>
            </>
...
const addS = StyleSheet.create({
  ...
  input: { borderRadius: radius.md, paddingHorizontal: spacing[4], paddingVertical: spacing[3], borderWidth: 1, fontSize: typography.base },
...
const makeStyles = (c, insets, isDark) => StyleSheet.create({
  ...
  floatingName: { fontSize: typography.base, fontWeight: '700' },
  ...
  sheet: { height: 280, borderTopLeftRadius: radius['2xl'], borderTopRightRadius: radius['2xl'], borderTopWidth: 1, overflow: 'hidden' },
  ...
  tab: { paddingHorizontal: spacing[3], paddingVertical: spacing[1], borderRadius: radius.pill, borderWidth: 1, borderColor: c.border, backgroundColor: c.surface2 },
  tabText: { fontSize: typography.xs, fontWeight: '600' },
  driverRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: spacing[4], paddingVertical: spacing[3], borderBottomWidth: 1, gap: spacing[3] },
  ...
  driverName: { fontSize: typography.sm, fontWeight: '600' },
  driverTruck: { fontSize: typography.xs, marginTop: 1 },
  driverRoute: { fontSize: 10, marginTop: 2 },
  statusBadge: { paddingHorizontal: spacing[2], paddingVertical: 1, borderRadius: radius.pill },
  statusBadgeText: { fontSize: 10, fontWeight: '700', textTransform: 'capitalize' },
  driverEta: { fontSize: 10 },
  chatHeader: { flexDirection: 'row', alignItems: 'center', padding: spacing[4], paddingTop: 56 },
  chatName: { color: '#fff', fontSize: typography.base, fontWeight: '700' },
  chatTruck: { color: 'rgba(255,255,255,0.7)', fontSize: typography.xs },
  msgWrap: { marginBottom: spacing[3] },
  msgRight: { alignItems: 'flex-end' },
  msgLeft: { alignItems: 'flex-start' },
  msgBubble: { maxWidth: '80%', borderRadius: radius.xl, padding: spacing[3] },
  msgText: { fontSize: typography.sm },
  msgTime: { fontSize: 10, marginTop: 4, textAlign: 'right' },
  chatInputRow: { flexDirection: 'row', padding: spacing[3], borderTopWidth: 1, gap: spacing[2] },
  chatTextInput: { flex: 1, borderRadius: radius.lg, paddingHorizontal: spacing[3], paddingVertical: spacing[2], borderWidth: 1, fontSize: typography.sm, maxHeight: 100 },
