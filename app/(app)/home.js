...
const ACTIVITY_ICONS = {
  LoadBooked: '📦', DriverDeparted: '🚛', LoadDelivered: '✅',
  DriverIdle: '⚠️', DocumentUploaded: '📄', StatusChanged: '🔄',
  load_created: '📦', driver_status: '🚛', load_delivered: '✅',
};

function normalizeStatus(d) {
  if (typeof d.status === 'number') return STATUS_LABELS[d.status] || 'offline';
...
export default function HomeScreen() {
  ...
  return (
    <PageBackground>
      ...
      >
        {/* Greeting banner */}
        <LinearGradient colors={['#4f46e5', '#6366f1', '#818cf8']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.banner}>
          <View>
          ...
          {hotLoads > 0 && (
            <View style={s.bannerAlert}>
              <Text style={s.bannerAlertText}>🔥 {hotLoads} hot</Text>
            </View>
          )}
        </LinearGradient>
          ...
          <View style={[s.alertsCard, { backgroundColor: glassFill, borderColor: glassBorder }]}>
            ...
            {hotLoads > 0 && (
              <TouchableOpacity style={[s.alertRow, { borderLeftColor: colors.danger }]} onPress={() => router.push('/(app)/loadboard')}>
                <View style={[s.alertIconWrap, { backgroundColor: colors.danger + '22' }]}>
                ...
                <Text style={[s.alertChevron, { color: colors.textDisabled }]}>›</Text>
              </TouchableOpacity>
            )}
            {idleDrivers > 0 && (
              <TouchableOpacity style={[s.alertRow, { borderLeftColor: colors.warning }]} onPress={() => router.push('/(app)/drivers')}>
                <View style={[s.alertIconWrap, { backgroundColor: colors.warning + '22' }]}>
        ...
        <View style={[s.card, { backgroundColor: glassFill, borderColor: glassBorder }]}>
          ...
          {drivers.slice(0, 4).map((d) => {
            ...
            const route = d.origin && d.dest ? `${d.origin} → ${d.dest}` : d.currentRoute || (st === 'offline' ? 'Offline' : 'En route');
            return (
          ...
          {activity.length > 0 ? activity.slice(0, 5).map((a, i) => (
            <View key={i} style={[s.activityRow, { borderTopColor: colors.borderSubtle }]}>
              <View style={[s.activityIconWrap, { backgroundColor: colors.surface2 }]}>
                <Text style={s.activityIcon}>{ACTIVITY_ICONS[a.type] || '🔔'}</Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text style={[s.activityText, { color: colors.textSecondary }]} numberOfLines={2}>
                  <Text style={{ fontWeight: '600', color: colors.textPrimary }}>{a.actor || a.actorName || 'System'}</Text>
                  {' — '}{a.target || a.description || a.message || a.type}
                </Text>
                <Text style={[s.activityTime, { color: colors.textDisabled }]}>{a.time || a.createdAt || 'recently'}</Text>
              </View>
            </View>
          )) : (
        ...
        <View style={[s.card, { backgroundColor: glassFill, borderColor: glassBorder }]}>
          ...
          {loads.slice(0, 5).map((l) => {
            ...
            const badgeBg = isHot ? colors.danger + '22' : isNew ? colors.accentMuted : colors.surface2;
            ...
            return (
              <View key={l.id} style={[s.loadRow, { borderTopColor: colors.borderSubtle }]}>
                ...
                <View style={{ alignItems: 'flex-end', gap: 3 }}>
                  <Text style={[s.loadRate, { color: l.rate > (l.marketRate || l.rate) ? colors.success : colors.textPrimary }]}>
        ...
        {/* Quick Actions */}
        <Text style={[s.sectionTitle, { color: colors.textPrimary, marginBottom: spacing[3] }]}>Quick Actions</Text>
        <View style={s.qaGrid}>
          {[
            { label: 'Track Drivers', icon: '🚛', gradient: ['#6366f1', '#4f46e5'], route: '/(app)/drivers' },
            { label: 'Loadboard',     icon: '📋', gradient: ['#8b5cf6', '#7c3aed'], route: '/(app)/loadboard' },
            { label: 'Documents',     icon: '📁', gradient: ['#06b6d4', '#0891b2'], route: '/(app)/documents' },
            { label: 'AI Assistant',  icon: '✦',  gradient: ['#f59e0b', '#d97706'], route: '/(app)/aichat' },
          ].map((qa) => (
            <TouchableOpacity key={qa.label} style={s.qaCard} onPress={() => router.push(qa.route)} activeOpacity={0.8}>
              <LinearGradient colors={qa.gradient} style={s.qaGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
...
const kpiS = StyleSheet.create({
  ...
  iconWrap: { width: 36, height: 36, borderRadius: radius.lg, justifyContent: 'center', alignItems: 'center', marginBottom: spacing[2] },
  icon: { fontSize: 18 },
  value: { fontSize: typography.xl, fontWeight: '800' },
  title: { fontSize: typography.xs, fontWeight: '600', marginTop: 2 },
...
const makeStyles = (c) => StyleSheet.create({
  ...
  bannerDate: { color: 'rgba(255,255,255,0.6)', fontSize: typography.xs, marginTop: 3 },
  bannerAlert: { backgroundColor: 'rgba(255,255,255,0.2)', paddingHorizontal: spacing[3], paddingVertical: spacing[2], borderRadius: radius.pill },
  bannerAlertText: { color: '#fff', fontSize: typography.xs, fontWeight: '700' },
  rangeRow: { flexDirection: 'row', borderRadius: radius.lg, padding: 3, marginBottom: spacing[4], borderWidth: 1 },
  rangeTab: { flex: 1, alignItems: 'center', paddingVertical: spacing[2], borderRadius: radius.md },
  rangeTabActive: { backgroundColor: c.accentMuted },
  ...
  alertRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: spacing[3], paddingLeft: spacing[3], borderLeftWidth: 3, marginTop: spacing[2], gap: spacing[3], borderRadius: 2 },
  alertIconWrap: { width: 34, height: 34, borderRadius: radius.md, justifyContent: 'center', alignItems: 'center' },
  ...
  driverRow: { flexDirection: 'row', alignItems: 'center', gap: spacing[3], paddingVertical: spacing[3], borderTopWidth: 1 },
  driverAvatar: { width: 38, height: 38, borderRadius: radius.pill, justifyContent: 'center', alignItems: 'center', position: 'relative' },
  driverInitials: { fontSize: typography.xs, fontWeight: '700' },
  driverDot: { width: 9, height: 9, borderRadius: 99, position: 'absolute', bottom: 0, right: 0, borderWidth: 1.5 },
  driverName: { fontSize: typography.sm, fontWeight: '600' },
  driverRoute: { fontSize: typography.xs, marginTop: 1 },
  statusBadge: { paddingHorizontal: spacing[2], paddingVertical: 2, borderRadius: radius.pill },
  statusBadgeText: { fontSize: 10, fontWeight: '700', textTransform: 'capitalize' },
  seeAll: { paddingTop: spacing[3], alignItems: 'center' },
  seeAllText: { fontSize: typography.sm, fontWeight: '600' },
  liveRow: { flexDirection: 'row', alignItems: 'center', gap: 5 },
  liveDot: { width: 7, height: 7, borderRadius: 99, backgroundColor: '#10b981' },
  liveText: { color: '#10b981', fontSize: typography.xs, fontWeight: '600' },
  activityRow: { flexDirection: 'row', gap: spacing[3], paddingVertical: spacing[3], borderTopWidth: 1 },
  activityIconWrap: { width: 34, height: 34, borderRadius: radius.md, justifyContent: 'center', alignItems: 'center' },
  ...
  loadRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: spacing[3], borderTopWidth: 1 },
